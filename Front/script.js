...
function updateCartModal() {
  ...
  cartTotal.textContent = convertToReal(total)

  cartCounter.innerText = cart.length;
}

function convertToReal(value) {
  coinsUpdated = value.toLocaleString("pt-Br", {
    style: "currency",
    currency: "BRL",
  });
  return coinsUpdated;
...
checkOutBtn.addEventListener("click", function () {
  ...
  const cartItems = cart.map((item) => {
    return `${item.name}, Quantidade: (${item.quantity}), Preço: ${convertToReal(item.price)} | `;
  }).join("");


  const message = encodeURIComponent(cartItems);
  const phone = "71992266662";
